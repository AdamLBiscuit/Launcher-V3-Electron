...
function updateSelectedAccount(authUser){
    let username = 'Aucun compte n`est selectionné'
    if(authUser != null){
...
function updateSelectedServer(serv){
    ...
    ConfigManager.save()
    server_selection_button.innerHTML = '\u2022 ' + (serv != null ? serv.getName() : 'Aucun serveur n`est selectionné')
    if(getCurrentView() === VIEWS.settings){
...
const refreshServerStatus = async function(fade = false){
    loggerLanding.log('Rafraichissement des données des serveurs')
    ...
    try {
        ...
        loggerLanding.warn('Impossible de rafraichir les données des seveurs. Ils sont surement hors ligne')
        loggerLanding.debug(err)
    }
    if(fade){
    ...
    sysAEx.on('message', (m) => {
        ...
        if(m.context === 'validateJava'){
            if(m.result == null){
                ...
                setOverlayContent(
                    '<br>Installation Java Introuvable',
                    'Pour rejoindre SAOFrance, vous avez besoin de Java 8 64-Bits. Voulez vous qu\'on vous l\'installe ? En installant, vous acceptez  <a href="http://www.oracle.com/technetwork/java/javase/terms/license/index.html">la license d\'installation d\'Oracle</a>.',
                    'Installer Java',
                ...
                setDismissHandler(() => {
                    $('#overlayContent').fadeOut(250, () => {
                        ...
                        setOverlayContent(
                            'Java est requis pour <br>lancer le jeu',
                            'Pour rejoindre SAOFrance, vous avez besoin de Java 8 64-Bits.',
                            'Je comprends',
                            'Retour'
                        )
                        setOverlayHandler(() => {
        ...
        if(m.context === 'validate'){
            switch(m.data){
                case 'distribution':
                    ...
                    setLaunchDetails('Chargement des données de la version..')
                    break
                case 'version':
        ...
        $.ajax(
            {
                ...
                success: (data) => {
                    ...
                    for (let i = 0; i < items.length; i++) {
                        ...
                        const link = "https://saofrance-mc.net/";
                        ...
                        let content = el.find('description\\:encoded').text()
